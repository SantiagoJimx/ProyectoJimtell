...
def editar_zapato(id: int, zapato: ZapatoCreate, db: Session = Depends(get_db)):
    return zapato_dao.editar_zapato(db, id, zapato)

#eliminar un zapato por su modelo
@router.delete("/zapatos/{id}", response_model=ZapatoResponse)
def eliminar_zapato(id: int, db: Session = Depends(get_db)):
    return zapato_dao.eliminar_zapato(db, id)
